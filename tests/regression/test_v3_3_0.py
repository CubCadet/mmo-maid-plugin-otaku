...
- /leaderboard [metric] slash command with `metric` ∈ {completed, score, hours}.
  (v11.0.0 amendment: renamed /otaku-leaderboard — the platform reserved
  /leaderboard for a built-in; the v3.3.0 contract otherwise stands.)
...
def test_manifest_includes_leaderboard():
    ...
    assert "otaku-leaderboard" in names


def test_leaderboard_metric_choices_frozen():
    cmd = next(c for c in _manifest()["slash_commands"] if c["name"] == "otaku-leaderboard")
...
def test_leaderboard_completed_query_filters_status():
    ...
    ctx.sql.query = _q
    p.cmd_leaderboard(ctx, _slash("otaku-leaderboard", {}, user_id="u"))
...
def test_leaderboard_score_query_uses_having_threshold():
    ...
    ctx.sql.query = _q
    p.cmd_leaderboard(ctx, _slash("otaku-leaderboard", {"metric": "score"}, user_id="u"))
...
def test_leaderboard_hours_uses_24min_heuristic():
    ...
    p.cmd_leaderboard(ctx, _slash("otaku-leaderboard", {"metric": "hours"}, user_id="u"))
